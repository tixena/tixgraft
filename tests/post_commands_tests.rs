...
fn execute_choice_with_match() {
    ...
    let command = PostCommand::Choice {
        options: vec![ChoiceOption::new(
            ...
            "version".to_owned(),
            Box::new(PostCommand::new("echo".to_owned(), vec!["matched!".to_owned()], None)),
        )],
    };
...
fn execute_choice_no_match() {
    ...
    let command = PostCommand::Choice {
        options: vec![ChoiceOption::new(
            ...
            Box::new(PostCommand::new("echo".to_owned(), vec!["should not run".to_owned()], None)),
        )],
    };
        ...
        options: vec![ChoiceOption::new(
            ...
            Box::new(PostCommand::Choice {
                options: vec![ChoiceOption::new(
                    ...
                    "inner".to_owned(),
                    Box::new(PostCommand::new("echo".to_owned(), vec!["nested match!".to_owned()], None)),
                )],
            }),
        )],
    };
...
fn execute_choice_with_regex_pattern() {
    ...
    let command = PostCommand::Choice {
        options: vec![ChoiceOption::new(
            ...
            r"v\d+\.\d+\.\d+".to_owned(),
            Box::new(PostCommand::new("echo".to_owned(), vec!["regex matched!".to_owned()], None)),
        )],
    };
...
fn complex_regex_patterns() {
    ...
    let command = PostCommand::Choice {
        options: vec![
            ChoiceOption::new(
                TestCommand::new("echo".to_owned(), vec!["ERROR: File not found".to_owned()], None),
                // Pattern: starts with ERROR, colon, space, any characters
                r"^ERROR:\s+.+".to_owned(),
                Box::new(PostCommand::new("echo".to_owned(), vec!["error detected!".to_owned()], None)),
            ),
            ChoiceOption::new(
                TestCommand::new("echo".to_owned(), vec!["WARNING: Low disk space".to_owned()], None),
                "^WARNING".to_owned(),
                Box::new(PostCommand::new("echo".to_owned(), vec!["warning detected!".to_owned()], None)),
            ),
        ],
    };
...
fn regex_case_sensitivity() {
    ...
    let command = PostCommand::Choice {
        options: vec![ChoiceOption::new(
            ...
            "version".to_owned(),
            Box::new(PostCommand::new("echo".to_owned(), vec!["should not match".to_owned()], None)),
        )],
    };
...
fn regex_with_word_boundaries() {
    ...
    let command = PostCommand::Choice {
        options: vec![ChoiceOption::new(
            ...
            Box::new(PostCommand::new("echo".to_owned(), vec!["version matched!".to_owned()], None)),
        )],
    };
    ...
    let commands = vec![PostCommand::Choice {
        options: vec![
            ChoiceOption::new(
                ...
                "anything".to_owned(),
                Box::new(PostCommand::new("echo".to_owned(), vec!["first option".to_owned()], None)),
            ),
            ChoiceOption::new(
                TestCommand::new("echo".to_owned(), vec!["success".to_owned()], None),
                "success".to_owned(),
                Box::new(PostCommand::new("echo".to_owned(), vec!["second option matched!".to_owned()], None)),
            ),
        ],
    }];
...
fn multiple_command_types_mixed_failures() {
    ...
    let commands = vec![
        ...
        PostCommand::Choice {
            options: vec![ChoiceOption::new(TestCommand::new("echo".to_owned(), vec!["test".to_owned()], None), "test".to_owned(), Box::new(PostCommand::new("echo".to_owned(), vec!["choice success".to_owned()], None)))],
        },
...
fn error_message_formatting() {
    ...
    let commands = vec![PostCommand::new("definitely_nonexistent_command_12345".to_owned(), vec!["arg1".to_owned()], None)];
